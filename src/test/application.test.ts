...
test("test useCountDownFormat", () => {
  expect(useApplication.useCountDownFormat(360000, "{dd}天{hh}时{mm}分{ss}秒")).toBe(
    "00天00时06分00秒"
  );
...
test("test useTimeFormat", () => {
  expect(
    useApplication.useTimeFormat(
      Number(new Date("2022-12-12 12:12:12")),
      "{YYYY}-{MM}-{dd} {hh}-{mm}-{ss}"
    )
  ).toBe("2022-12-12 12-12-12");
  expect(
    useApplication.useTimeFormat(Number(new Date("2022-12-12 12:2:12")), "{hh}-{mm}-{ss}")
  ).toBe("12-02-12");
  expect(
    useApplication.useTimeFormat(
