 ...
 */
export const usePhoneLegal = (phone: number | string, legal: RegExp = /^[1][3,4,5,7,8][0-9]{9}$/) => {
 ...
 */
export const useIDCargLegal = (idCard: string) => {
 ...
 */
export const useEmailLegal = (email: string, legal: RegExp = /^[A-Za-z\d]+([-_.][A-Za-z\d]+)*@([A-Za-z\d]+[-.])+[A-Za-z\d]{2,4}$/) => {
  if (legal.test(email)) {
    return true;
  } else {
    return false;
