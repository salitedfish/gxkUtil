...
type PromiseQueueOptions = { max?: number; interval?: number; insist?: boolean };
  ...
  return new Promise((resolve, reject) => {
    ...
    const handler = async () => {
      for (const promiseHandler of promiseList) {
        ...
        const _promiseHandler = options.insist ? usePromiseInsist(promiseHandler, options, condition)[0] : promiseHandler;
